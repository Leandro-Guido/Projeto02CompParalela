// gcc -fopenmp -o par.exe main.c -lm && ./par.exe 1 "adults_0.125.csv" 2 0.01 15 15
...
void print_network(Network *net)
{
    ...
    for (int l = 0; l < net->n_layers; l++)
    {
        ...
        for (int n = 0; n < layer->n_neurons; n++)
        {
            ...
            for (int w = 0; w < neuron->n_weights; w++)
            {
                printf("%.4f", neuron->weights[w]);
                if (w < neuron->n_weights - 1)
                    printf(", ");
            }
            printf("], activation = %.4f, output = %.4f, delta = %.4f\n", neuron->activation, neuron->output, neuron->delta);
        }
    }
}
...
void forward_propagate(Network *net, float *inputs)
{
    ...
    for (int l = 0; l < net->n_layers; l++)
    {
        ...
        for (int n = 0; n < layer->n_neurons; n++)
        {
            ...
            neuron->output = sigmoid(neuron->activation);
...
void train(Network *net, float **train_data, int train_rows, float l_rate, int n_epoch, int n_outputs, int id)
{
    int cols = net->layers[0].neurons[0].n_weights;
